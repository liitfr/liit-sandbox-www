...
const animOnScroll = require('./_animOnScroll.js')
...
IsoGrid.prototype._init = function () {
  ...
  imagesLoaded(this.gridEl, function () {
    ...
    if (self.options.type === 'scrollable') {
      new animOnScroll.AnimOnScroll(self.gridEl, {
        minDuration: 1,
        maxDuration: 1.2,
        viewportFactor: 0
      })
...
IsoGrid.prototype._expandSubItems = function (item) {
  var self = this
  var itemLink = item.querySelector('.grid__link')
  var subItems = [].slice.call(itemLink.querySelectorAll('.layer'))
  var subItemsTotal = subItems.length
...
function generateIsoLayer () {
  [].slice.call(document.querySelectorAll('.isolayer')).forEach(function (el) {
    new IsoGrid(el, {
      ...
      stackItemsAnimation: {
        properties: function (pos) {
          var rotateX = getRandomInt(-73, -67)
          if (pos === 0) {
            rotateX = getRandomInt(-33, -27)
          } else if (pos === 1) {
            rotateX = getRandomInt(-53, -47)
          }
          var translateZ = getRandomInt(145, 155)
          if (pos === 0) {
            translateZ = getRandomInt(45, 55)
          } else if (pos === 1) {
            translateZ = getRandomInt(95, 105)
          }
          return {
            rotateZ: getRandomInt(-3, 3),
            rotateX: rotateX,
            rotateY: getRandomInt(32, 38),
            translateZ: translateZ,
            translateY: getRandomInt(-95, -105)
          }
        },
        options: function (pos, itemstotal) {
          return {
      ...
      onGridLoaded: function () {
        $('#internetkf').addClass('grid-loaded')
      }
    })
  })
}

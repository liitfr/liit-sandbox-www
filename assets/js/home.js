...
function staticifyHeight () {
  $('.part').height(
    window.innerHeight * 2 < $('.part').css('min-height').replace('px', '')
    ? $('.part').css('min-height').replace('px', '')
    : ($('.part').height(
        window.innerHeight * 2 > $('.part').css('max-height').replace('px', '')
        ? $('.part').css('max-height').replace('px', '')
...
$(window).resize(function () {
  var progress = sceneMoveLogo.progress()
...
new ScrollMagic.Scene({
  ...
  .setTween(TweenMax.to('#logo-banner .polylogo', 2, {fill: '#FFF'}))
  .addTo(smController)
...
$('.part').not('.first').each(function (idx, ele) {
  new ScrollMagic.Scene({
    ...
    .setTween(TweenMax.to('#logo-banner text', 2, {fill: $(ele).css('background')}))
    .addTo(smController)
})
